...
ligand = "/truba/home/iakgun/TMY/pdbqt_1/"
...
dos_2 = res + local[:-6]
os.mkdir(dos_2)
shutil.copy(res + local, dos_2)
with open (dos_2 + "/config_"+ local[:-6] + ".txt", "a") as dosya_3:
    dosya_3.write("receptor = "  + res + "res/receptor.pdbqt\n")
...
dos_3 = res + local[:-6] + "_loc"
os.mkdir(dos_3)
shutil.copy(res + local, dos_3)
with open (dos_3 + "/config_"+ local[:-6] + "_loc"+ ".txt", "a") as dosya_4:
    dosya_4.write("receptor = " + res + "res/receptor.pdbqt\n")
